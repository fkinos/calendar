...
draw_string (unsigned int *pixels, int x, int y, char *string, unsigned int color)
{
  ...
  for (i = 0; i < (int)strlen (string); i++) {
    draw_letter (pixels, x + (i * 8), y, string[i], color);
  }
}
...
draw ()
{
  ...
  sprintf (heading, "%s %d", months[tm->tm_mon], tm->tm_year + 1900);
  draw_string (pixels, 10, 10, heading, BLACK);

  draw_line (pixels, 10, 24, WIDTH - 12, 24, BLACK);

  /* draw week days */
  for (i = 0; i < 7; i++) {
    draw_string (pixels, 10 + (i * 40), 40, weekdays[i], BLACK);
  }
  ...
  for (i = 0; i < 7; i++) {
    for (j = 0; j < 6; j++) {
      if (calendar[j][i] != 0) {
        ...
        draw_string (pixels, 14 + (i * 40), 56 + (j * 15), day_string, color);
      }
    }
  }
}
